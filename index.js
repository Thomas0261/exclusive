...
app.get("/", (req, res) => {
  res.send("🚗 Backend API (SMS + Email) is running");
});
...
Notes: ${notes || "None"}`;
  ...
  const clientText = `Hi ${firstName}, your reservation for "${service}" on ${date} at ${time} is confirmed. We’ll contact you soon. – Exclusive Town Cars`;
  ...
  const emailHTML = `
    ...
    <p><strong>Car Seats:</strong> ${carSeats || "N/A"}</p>
    <p><strong>Service:</strong> ${service}</p>
    <p><strong>Notes:</strong><br>${notes || "None"}</p>
    <hr><small>Sent at ${now}</small>
  `;
  ...
  try {
    ...
    res.status(200).json({ success: true, message: "SMS and Email sent" });
  } catch (err) {
